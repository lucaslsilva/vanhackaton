...
{
    public abstract class TaskManagerTestBase : AbpIntegratedTestBase<TaskManagerTestModule>
    {
        ...
        protected void UsingDbContext(Action<TaskManagerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TaskManagerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TaskManagerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TaskManagerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TaskManagerDbContext> action)
        {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TaskManagerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaskManagerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TaskManagerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TaskManagerDbContext, Task<T>> func)
        {
            T result;
